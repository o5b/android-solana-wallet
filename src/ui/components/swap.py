...
async def go_to_swap_page_click(ctx: AppContext, e) -> None:
    ...
    if data['network'] != _MAINNET:
        page.show_dialog(
            flet.AlertDialog(title=flet.Text(ctx.t("swap_mainnet_only")))
        )
        return
    if not ctx.has_wallet_private_key(data['wallet_data']):
        page.show_dialog(
            flet.AlertDialog(title=flet.Text(ctx.t("swap_needs_key")))
        )
        return
    dd_in = flet.Dropdown(
        label=ctx.t("swap_input_token"), value="SOL", width=200,
        options=[flet.dropdown.Option(sym) for sym in SWAP_TOKENS],
    )
    dd_out = flet.Dropdown(
        label=ctx.t("swap_output_token"), value="USDC", width=200,
        options=[flet.dropdown.Option(sym) for sym in SWAP_TOKENS],
    )
    tf_amount = flet.TextField(label=ctx.t("amount_label"), width=200, max_length=30)
    tf_slippage = flet.TextField(label=ctx.t("slippage_pct"), value="1.0", width=120, max_length=6)
    txt_quote = flet.Text(value=ctx.t("swap_enter_amount"), selectable=True)
    ...
    async def get_quote_button_click(ev):
        ...
        try:
            if dd_in.value == dd_out.value:
                txt_quote.value = ctx.t("tokens_must_differ")
                page.update(); return
            amount_str = (tf_amount.value or "").strip()
            if not is_valid_amount(amount_str):
                txt_quote.value = ctx.t("invalid_amount_short"); page.update(); return
            ...
            if in_raw <= 0:
                txt_quote.value = ctx.t("amount_gt_zero"); page.update(); return
            ...
            txt_quote.value = (
                ...
                + ctx.t("min_received", amount=f"{min_out:.6f}", sym=dd_out.value)
                + "\n"
                + ctx.t("price_impact", pct=f"{float(q.get('priceImpactPct', 0)) * 100:.3f}")
            )
    ...
    async def swap_button_click(ev):
        ...
        try:
            if await_holder["quote"] is None:
                txt_quote.value = ctx.t("press_quote_first"); page.update(); return
            if dd_in.value == dd_out.value:
                txt_quote.value = ctx.t("tokens_must_differ"); page.update(); return
            ...
            if changed:
                txt_quote.value = ctx.t("swap_inputs_changed"); page.update(); return
            ev.control.disabled = True
            txt_quote.value = ctx.t("swapping_wait")
            page.update()
            in_mint = SWAP_TOKENS[dd_in.value][0]
            out_mint = SWAP_TOKENS[dd_out.value][0]
            res = await jup_swap(
            ...
            err = conf.get("err")
            if err:
                txt_quote.value = ctx.t("swap_failed", err=err, sig=res['signature'])
            else:
                txt_quote.value = ctx.t(
                    "swap_success", status=status,
                    received=f"{out_ui:.6f}", sym=dd_out.value, sig=res['signature'],
                )
        except Exception as er:
            import traceback
            print(f"[SWAP] swap ERROR: {er}\n{traceback.format_exc()}")
            txt_quote.value = ctx.t("swap_error", err=er)
        finally:
    ...
    el_swap_page.controls.extend([
        flet.Row([flet.Text(
            value='',
            spans=[
                flet.TextSpan(ctx.t("lbl_wallet"), flet.TextStyle(size=16)),
        ...
        flet.Row([
            flet.ElevatedButton(ctx.t("get_quote"), on_click=get_quote_button_click),
            flet.ElevatedButton(ctx.t("swap_btn"), on_click=swap_button_click),
        ]),
...
def build_swap_page(ctx: AppContext) -> flet.View:
    ...
    return flet.View(
        route="swap-page",
        appbar=flet.AppBar(
            title=flet.Text(ctx.t("swap_appbar_title")),
            color="white",
            bgcolor="green",
        ...
        controls=[
            flet.Text(ctx.t("swap_heading"), size=30, font_family="Georgia"),
